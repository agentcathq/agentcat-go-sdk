...
mcpgo/                      # mcp-go adapter (separate go.mod, depends on mark3labs/mcp-go)
  ...
  registered_schema.go      # declares _mcp_instructions on registered output schemas
...
**Explicit handles, no sessions.** Each advertised tool gains an optional `session_id` parameter (never `required`), plus `agent_id` (required, only when `EnableAgentTracking`) and `context` — subject to the per-tool skip rules noted below. On a call with no `session_id` the SDK mints one and appends a trailing `[MCP INSTRUCTIONS]` text block to the wire result — only on the minting call. Tools whose declared output schema was extended also get an `_mcp_instructions` mirror in `structuredContent` on every response (minting instructions on the minting call, handle confirmation after). All of this is **wire-only**: published events always carry the customer's raw unstripped request and their original undecorated response. Injected parameters (`session_id`, `agent_id`, `context`) are stripped from the arguments before the customer's handler runs. Configuring `Options.ResolveSessionID` switches to **hook mode**: no `session_id` is injected anywhere and no session instructions are ever shown to the agent.
